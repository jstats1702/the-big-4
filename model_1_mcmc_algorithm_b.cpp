     ...
     for (int k = 0; k < K; ++k) {
          ...
          for (int i = 0; i < n - 1; ++i) {
               ...
               for (int j = i + 1; j < n; ++j) {
                    ...
                    const double mean_ijk = zeta + d_ik + d_jk;
...
Rcpp::List gibbs_sampler_multilayer(const arma::cube& y, 
     ...
     for (int iter = 1; iter <= n_iter; ++iter) {
          ...
          z        = sample_z_cpp(y, delta, zeta, z, K, n);
          zeta     = sample_zeta_cpp(z, delta, omega2, K, n);
          delta    = sample_delta_cpp(z, zeta, tau2, delta, vartheta, K, n);
...
arma::vec log_likelihood_multilayer_cpp(const arma::cube& y, 
     ...
     for (int s = 0; s < n_samples; ++s) {
          ...
          for (int k = 0; k < K; ++k) { 
               ...
               for (int i = 0; i < n - 1; ++i) {
                    for (int j = i + 1; j < n; ++j) {
                         double eta_ijk = zeta_s + delta_k[i] + delta_k[j];
...
double log_likelihood_iter_cpp(const arma::cube& y,
     ...
     for (int k = 0; k < K; ++k) {
          ...
          for (int i = 0; i < n - 1; ++i) {
               ...
               for (int j = i + 1; j < n; ++j) {
                    const double d_jk = del_k[j];
                    const double eta  = zeta + d_ik + d_jk;
...
arma::vec log_likelihood_pointwise_cpp(const arma::cube& y,
     ...
     for (int k = 0; k < K; ++k) {
          ...
          for (int i = 0; i < n - 1; ++i) {
               ...
               for (int j = i + 1; j < n; ++j) {
                    const double d_jk    = del_k[j];
                    const double eta_ijk = zeta + d_ik + d_jk;
...
// [[Rcpp::export]]
arma::cube interaction_prob_cpp(const arma::mat& delta,
                                const double     zeta,
                                const int        K) {
...
// [[Rcpp::export]]
arma::cube simulate_multilayer_network_cpp(const arma::mat& delta,
                                           const double     zeta,
                                           const int        K) {
     
     const int n = delta.n_rows;
     
     arma::cube P = interaction_prob_cpp(delta, zeta, K);
