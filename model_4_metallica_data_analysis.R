...
load("samples_model_4_metallica.RData")
...
MCE <- c(
     ...
     mean(apply(samples$lambda,   2,       mce)),
     mean(apply(samples$U,        c(2,3),  mce)),
     mce(samples$omega2),
     mce(samples$sigma2),
     mce(samples$tau2),
     mce(samples$kappa2),
     mce(samples$varsigma2),
     mce(samples$upsilon2)
)
names(MCE) <- c("zeta", "mu", "delta", "vartheta", "beta", "lambda", "U", 
                "omega2", "sigma2", "tau2", "kappa2", "varsigma2", "upsilon2")
...
tab <- rbind(
     ...
     summ_vec(sqrt(samples$upsilon2))
     
)
rownames(tab) <- c("omega", "sigma", "tau", "kappa", "varsigma", "upsilon")
...
# Inference on bilinear interaction weights ====================================

x11()
plot_lambda_by_layer(exp(samples$lambda))

tab <- cbind(
     apply(samples$lambda, 2, mean),
     apply(samples$lambda, 2, sd),
     t(apply(samples$lambda, 2, quantile, probs = c(0.025, 0.975)))
)
rownames(tab) <- c("Loudness", "Brightness", "Tonality", "Rhythm")
colnames(tab) <- c("Mean", "SD", "Q2.5", "Q97.5")
...
# Inference on latent positions norms ==========================================

x11()
plot_u_norms(samples$U)

# Inference on geometry geometry strength ======================================

x11()
plot_geometry_strength_by_layer(samples$U, exp(samples$lambda))

# Latent positions =============================================================

x11()
plot_latent_positions(samples)
...
dic_model_4(log_lik, samples, y, X)

waic_model_4(samples, y, X)

# Test statistics ==============================================================

x11()
plot_test_stats_model_4(samples, y, X, layer_titles = c(1,2,3,4))

# Originality score ============================================================

originality_by_layer(samples, thin = 50)
