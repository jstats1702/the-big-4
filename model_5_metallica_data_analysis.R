...
load("samples_model_5_metallica.RData")
...
MCE <- c(
     ...
     mean(apply(samples$Theta,    c(1,2),  mce)),
     mean(apply(samples$omega,    c(2,3),  mce)),
     mce(samples$alpha),
     mce(samples$omega2),
     mce(samples$sigma2),
     mce(samples$tau2),
...
names(MCE) <- c("zeta", "mu", "delta", "vartheta", "beta", "Theta", "omega", "alpha",
                "omega2", "sigma2", "tau2", "kappa2", "varsigma2", "rho2")
...
tab <- rbind(
     ...
     summ_vec(sqrt(samples$rho2))
     
)
rownames(tab) <- c("omega", "sigma", "tau", "kappa", "varsigma", "rho")
...
dic_model_5(log_lik, samples, y, X)

waic_model_5(samples, y, X)

# Test statistics ==============================================================

x11()
plot_test_stats_model_5(samples, y, X, layer_titles = c(1,2,3,4))
