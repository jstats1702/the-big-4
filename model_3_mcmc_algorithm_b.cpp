...
arma::mat sample_U_cpp(const arma::cube& z,
     ...
     for (int i = 0; i < n; ++i) {
          ...
          for (int k = 0; k < K; ++k) {
               ...
               for (int j = 0; j < n; ++j) {
                    if (j != i) {
                         const double    r_ijk = z(i, j, k) - mu_k - zeta - delta(i, k) - delta(j, k);
...
Rcpp::List gibbs_sampler_multilayer(const arma::cube& y,
     ...
     for (int iter = 1; iter <= n_iter; ++iter) {
          ...
          lambda    = sample_lambda_cpp(z, mu, delta, U, zeta, upsilon2, lambda, K, n);
          U         = sample_U_cpp(z, mu, delta, lambda, zeta, U, K, n, d);
